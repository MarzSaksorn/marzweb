...
export default function Home() {
  return (
    <div className="space-y-6">
      {/* Hero */}
      <Card className="border-[#A78BFA]/20 overflow-hidden relative animate-up">
        ...
        <CardHeader className="relative">
          ...
          <CardDescription className="text-sm max-w-2xl leading-relaxed">
            A personal showcase of my journey in software development —
            from web apps to games and creative experiments.
          </CardDescription>
        </CardHeader>
        <CardContent className="relative">
          <Button className="bg-[#A78BFA] hover:bg-[#8B6FE8] text-white press" asChild>
            <a href="/projects">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {featured.map((project) => (
              <Card key={project.id} className="group hover:-translate-y-[1px] transition-all duration-200 border-border animate-up">
                ...
                <CardFooter className="p-4 pt-0 gap-2">
                  {project.githubUrl && (
                    <Button variant="outline" size="sm" className="text-xs border-border press" asChild>
                  ...
                  {project.liveUrl && (
                    <Button size="sm" className="text-xs bg-[#A78BFA] hover:bg-[#8B6FE8] text-white press" asChild>
        ...
        <div className="space-y-4">
          ...
          {currentProject && (
            <Card className="border-[#A78BFA]/20 animate-up">
              <div className="relative h-28 overflow-hidden rounded-t-[8px]">
                ...
                <div className="absolute bottom-2 left-3">
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-[#A78BFA] text-white">
              ...
              <CardHeader className="p-3">
                ...
                <CardDescription className="text-xs line-clamp-2">{currentProject.description}</CardDescription>
              </CardHeader>
              <CardFooter className="p-3 pt-0">
                <Button size="sm" className="text-xs bg-[#A78BFA] hover:bg-[#8B6FE8] text-white w-full press" asChild>
          ...
          <Card className="animate-up">
            <CardHeader className="p-3">
              <CardTitle className="text-xs font-semibold flex items-center gap-1.5">
                <Code2 className="h-3.5 w-3.5 text-[#A78BFA]" /> Tools
              </CardTitle>
            </CardHeader>
            <CardContent className="p-3 pt-0">
              <div className="flex flex-wrap gap-1.5">
                {techStack.map((tech) => (
                  <span key={tech} className="px-2 py-0.5 rounded-full text-[11px] bg-[#A78BFA]/10 text-[#A78BFA] dark:text-[#C4B5FD]">
                    {tech}
                  </span>
                ))}
              </div>
